...
export interface ClientToServerEvents {
  /** Join a room by its code. Server responds with lobby:state or lobby:error. */
  'lobby:join': (payload: { roomCode: string; displayName: string }) => void;

  /** Leave the current room gracefully. */
  'lobby:leave': () => void;

  /** Host starts the game (only valid when status === 'waiting'). */
  ...
  /**
   * Busting player invokes their Second Chance card to survive a duplicate.
