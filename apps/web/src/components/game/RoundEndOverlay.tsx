...
export default function RoundEndOverlay({ roundNumber, roundScores, cumulativeScores, flip7PlayerId, playerNames }: Props) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setCountdown((c) => {
        if (c <= 1) { clearInterval(id); clearRoundEnd(); return 0; }
        return c - 1;
      });
    }, 1000);
          ...
          {sorted.map(([pid, total], i) => {
            ...
            return (
              <div
                ...
                style={{ background: i === 0 ? 'rgba(251,191,36,0.08)' : 'rgba(255,255,255,0.03)' }}
              >
                <span
                  className="text-sm font-medium truncate"
                  style={{ fontFamily: 'var(--font-fredoka)', color: i === 0 ? 'var(--color-flip7-gold)' : '#f1f5f9' }}
                >
                  {i === 0 ? '★ ' : ''}{playerNames[pid] ?? pid}
                </span>
                <span
                  className="text-sm text-right"
                  style={{ fontFamily: 'var(--font-fredoka)', color: round === 0 ? '#6b7280' : '#4ade80' }}
                >
                  +{round}
                </span>
                <span
