...
export default function SelectTargetModal({ action, validTargetIds, expiresAt, roomId, players }: Props) {
  const clearSelectTargetPrompt = useGameStore((s) => s.clearSelectTargetPrompt);
  ...
  function handleSelect(targetPlayerId: string) {
    getGameSocket('', '').emit('game:select_action_target', { roomId, targetPlayerId });
    clearSelectTargetPrompt();
  }
  ...
  const validTargets = players.filter((p) => validTargetIds.includes(p.playerId));
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <p
            ...
            style={{ fontFamily: 'var(--font-fredoka)', color: actionColor[action] }}
          >
            {actionLabel[action]} — Choose a target
          </p>
          <span className="text-sm font-bold" style={{ fontFamily: 'var(--font-fredoka)', color: '#94a3b8' }}>
            {secs}s
          </span>
        </div>
        ...
        <div className="h-1 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.1)' }}>
          <div
            ...
            style={{ width: `${pct}%`, background: actionColor[action], transition: 'width 0.1s linear' }}
          />
        </div>
