...
const statusConfig: Record<PlayerStatus, { label: string; color: string; icon: string }> = {
...
export default function PanelHeader({
  rank, displayName, status, roundScore, totalScore, flipThreeRemaining, isYou = false,
}: Props) {
  const cfg = statusConfig[status];
  const isFlipThree = status === 'active' && flipThreeRemaining !== undefined && flipThreeRemaining > 0;
